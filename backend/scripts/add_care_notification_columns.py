...
`app/models/notification.py`) needs for the missed-medicine and
appointment-reminder notifications — `medicine_id`, `dose_time`,
`appointment_id` — to an existing `notifications` table.
...
Usage (from `backend/`, with the same environment/`.env` the API uses):
    python scripts/add_care_notification_columns.py
...
def main() -> None:
    ...
    if not to_add:
        print("Nothing to do — 'notifications' already has every care-notification column.")
        return
