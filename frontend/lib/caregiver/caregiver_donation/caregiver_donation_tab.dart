...
/// Donation landing content for the caregiver's Donation tab: a
/// CareConnect Central Fund CTA card plus a link into the donation
/// history — both routing into the existing, shared donation pages
/// rather than duplicating their data here.
///
/// No Scaffold/AppBar of its own, since this is embedded directly as a
/// bottom-nav tab body (see caregiver_dashboard_page.dart).
class CaregiverDonationTab extends StatelessWidget {
  const CaregiverDonationTab({super.key});
...
class _DonationHistoryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      // A paler tint of paleMint rather than the default theme color
      // (which was rendering pink) — keeps it on-brand but clearly
      // secondary to the Central Fund card above.
